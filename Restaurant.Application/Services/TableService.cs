...
public class TableService : ITableService
{
    ...
    public async Task<ApiResponse<string>> GetQRCodeImageAsync(Guid tableId, string? baseUrl)
    {
        ...
        var url = string.IsNullOrEmpty(baseUrl)
            ? $"menu.html?tableId={tableId}"
            : $"{baseUrl}/menu.html?tableId={tableId}";

        var qrBase64 = _qrCodeService.GenerateQRCode(url);
